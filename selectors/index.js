...
export const getSceneSlugs = stream => 
	getScenes(stream).map(s => s.slug)
export const getSceneBySlug = (stream, scene) =>
	getScenes(stream).find(s => s.slug === scene)
export const getSceneByIndex = (stream, index) =>
	getScenes(stream)[index]
...
export const getText = (locale, type, slug, ...key) => {
	try {
		...
		return obj ? (typeof key[0] === 'boolean' ? obj : obj[key.join('_') || 'text']) : ''
	} catch (error) {
		console.warn(error)
		return null
	}
