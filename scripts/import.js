...
const LANG_KEYS = ['en']
const STREAMS = ['glass', 'landfill', 'metal', 'organics', 'paper', 'plastic']
const DATA_SCHEMA = {
	follow: STREAMS,
	sort: ['items'],
}
...
const getFilePaths = (dir) =>
	fs.readdirSync(dir, { withFileTypes: true })
		.filter(dirent => dirent.isDirectory() || path.extname(dirent.name) === '.csv')
	    .map(dirent => {
...
const writeJson = (json, name) => {
	const filePath = path.join(__dirname, '..', 'constants', `${name}.js`)
