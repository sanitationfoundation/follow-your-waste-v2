...
const FollowVoices = ({ stream }) => {
	...
	const resetAudio = useCallback(slug => {
		const audioElem = audios.current[slug]
		if(audioElem)
			audioElem.currentTime = 0
	}, [audios])

	const playAudio = useCallback(slug => {
		const audioElem = audios.current[slug]
		if(audioElem)
			audioElem.play()
	...
	const pauseAudio = useCallback(slug => {
		const audioElem = audios.current[slug]
		if(audioElem)
			audioElem.pause()
	}, [audios])
	...
	useEffect(() => {
		Object.values(audios.current).forEach(audio => audio.volume = .75)
	}, [audios])

	useEffect(() => {
		if(currVoice !== prevVoice)
			setNewVoice(currVoice)
